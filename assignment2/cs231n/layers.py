...
def affine_forward(x, w, b):
    ...
    ###########################################################################
    #pass
    product = np.prod(np.array(x[-1].shape))
    #print(product)
    N = x.shape[0]
    #print(N)
    temp_x = np.reshape(x,(N,product))
    #print(temp_x.shape)
    #print(x.shape)
...
def batchnorm_backward(dout, cache):
    ...
    #pass
    N, D = dout.shape
    xmew, sample_mean, sample_var, new_x, gamma, beta, eps, xmew, ivar, denomi = cache
    dbeta = np.sum(dout,axis=0)
    dy = dout
    dgamma = np.sum(dy * new_x, axis=0)
    dxhat = dy * gamma
    divar = np.sum(dxhat * xmew, axis=0)
    dsub1 = dxhat * ivar
    dsqrt = divar * (-1) * (ivar**2)
    dvar = 0.5 * ivar * dsqrt
    dsq = 1. /N * np.ones((N,D)) * dvar
    dsub2 = 2 * xmew * dsq
    dmean = -1 * np.sum(dsub1 + dsub2,axis=0)
    dx1 = 1 * (dsub1 + dsub2)
...
def batchnorm_backward_alt(dout, cache):
    ...
    dxhat = dout * gamma
    dx = 1. / N * ivar * (N*dxhat - np.sum(dxhat,axis = 0) - new_x * (np.sum(dxhat * new_x,axis = 0)))
    ###########################################################################
    #                             END OF YOUR CODE                            #
    ###########################################################################
...
def conv_forward_naive(x, w, b, conv_param):
    ...
    ###########################################################################
    #pad_x = np.pad(x,(conv_param['pad']),'constant', constant_values=(0))
    N, C, H, W = x.shape
    F, C, HH, WW = w.shape
    pad = conv_param['pad']
...
def max_pool_forward_naive(x, pool_param):
    ...
    Wout = int(1 + (W - Fw)/stride)
    Hout = int(1 + (H - Fh)/stride)
    out= np.zeros((N, C, Hout, Wout))
    x_ = x.reshape(N * C, 1, H, W)
    x_col = im2col_indices(x_, Fh, Fw, padding=0, stride=stride)
    ...
    #                             END OF YOUR CODE                            #
    ###########################################################################
    cache = (x, pool_param, x_col, maxidx)
    return out, cache
...
def max_pool_backward_naive(dout, cache):
    ...
    ###########################################################################
    #pass
    (x, pool_param, x_col, maxidx) = cache
    N, C, H, W = x.shape
    Fh = pool_param['pool_height']
    Fw = pool_param['pool_width']
    stride = pool_param['stride']
    dx_cols = np.zeros((x_col.shape))
    dout_flat = dout.transpose(2,3,0,1).ravel()
    dx_cols[maxidx, range(maxidx.size)] = dout_flat
    dx = col2im_indices(dx_cols, (N * C, 1, H, W), Fh, Fw, padding=0, stride=stride)
